/*
 * Node_A_Spk_Mic_Test_48k_idf
 *
 * Bisection Test B for the bridge speaker bug.
 *
 * Identical to Node_A_Spk_Mic_Test_idf (Test C, which works end-to-end)
 * EXCEPT the UAC sample rate is 48 kHz instead of 44.1 kHz. Speaker is
 * still STEREO, mic is still mono silent, wire format is still 240 B
 * ADPCM packets to Node_B_Audio.ino (unchanged).
 *
 * What we're answering:
 *   Test C proved CFG_TUD_AUDIO_FUNC_1_N_AS_INT=2 (mic+spk) works fine
 *   at 44.1 kHz stereo. The bridge fails at 48 kHz mono. This test
 *   isolates whether the 48 kHz rate alone is the trigger.
 *
 * Interpretation:
 *   WHITE LED (audio may sound slowed/pitched on Node B because the
 *   B decoder still expects 44.1 kHz; that's irrelevant for diagnosis)
 *     -> 48 kHz is fine. Bug is the mono speaker channel count.
 *   GREEN LED only, no audio
 *     -> 48 kHz with mic enabled is the trigger. Look into TinyUSB iso
 *        EP scheduling and feedback EP behavior at 48 kHz, or audio
 *        function buffer math that depends on rate.
 *   BLUE blink only with arecord+aplay running
 *     -> host never opens streams. Inspect descriptor for 48 kHz issues.
 *
 * Wire format is identical to Test C / audio_idf / Node_A_Audio.ino:
 *   240 B ADPCM = 240 stereo pairs, codeL low nibble, codeR high.
 *   Since UAC is 48 kHz here but Node B decodes at 44.1 kHz, samples
 *   play 48000/44100 = 1.088x slower. This is expected, ignore it.
 */
...
static const char *TAG = "spk_mic_48k";
...
/* IMA ADPCM tables -- identical to Test C / audio_idf. */
...
void app_main(void)
{
    ESP_LOGI(TAG,
             "Node_A_Spk_Mic_Test_48k_idf "
             "(48 kHz stereo spk + 48 kHz mono silent mic, ESP-NOW ch %d)",
             ESP_NOW_CHANNEL);
    ...
    ESP_LOGI(TAG, "UAC ready (spk + silent mic, both @ 48 kHz)");
