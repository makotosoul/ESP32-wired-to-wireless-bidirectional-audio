#!/bin/bash

# Handle Ctrl+C (SIGINT) to stop the script immediately
trap "echo -e '\nExiting...'; exit" INT

# Configuration
BASE_DIR="/home/makotosoul/project/Thesis"
NODE_A_PROJECT="$BASE_DIR/Node_A_Mic_Test_v2_idf"  # PlatformIO + ESP-IDF
NODE_B_SKETCH="$BASE_DIR/Node_B_Mic_Test"          # Arduino (A1S Audio Kit)

require() {
    command -v "$1" >/dev/null 2>&1 || {
        echo "Error: '$1' not found in PATH. Install it and re-run."
        exit 1
    }
}
require arduino-cli
require jq
require pio

echo "Scanning for connected boards for Microphone Test..."
JSON_DATA=$(arduino-cli board list --format json)
# Extract only ports that have a serial protocol
PORTS=($(echo "$JSON_DATA" | jq -r '.detected_ports[] | select(.port.protocol=="serial") | .port.address'))

get_board_name() {
    local port=$1
    echo "$JSON_DATA" | jq -r ".detected_ports[] | select(.port.address==\"$port\") | .matching_boards[0].name // \"Unknown Board\""
}

get_vid() {
    local port=$1
    echo "$JSON_DATA" | jq -r ".detected_ports[] | select(.port.address==\"$port\") | .port.properties.vid // \"\""
}
get_pid() {
    local port=$1
    echo "$JSON_DATA" | jq -r ".detected_ports[] | select(.port.address==\"$port\") | .port.properties.pid // \"\""
}

is_s3_port() {
    local port=$1
    [ "$(get_vid $port)" == "0x303a" ] && [ "$(get_pid $port)" == "0x1001" ]
}

get_fqbn() {
    local port=$1
    if is_s3_port "$port"; then
        # Should not be used now (S3 is flashed via PlatformIO), but kept for symmetry.
        echo "esp32:esp32:esp32s3:CDCOnBoot=cdc,USBMode=default,FlashMode=qio,FlashSize=16M,PSRAM=opi"
        return
    fi
    local fqbn=$(echo "$JSON_DATA" | jq -r ".detected_ports[] | select(.port.address==\"$port\") | .matching_boards[0].fqbn // \"\"")
    if [ "$fqbn" == "null" ] || [ -z "$fqbn" ] || [ "$fqbn" == "esp32:esp32:esp32_family" ]; then
        fqbn="esp32:esp32:esp32"
    fi
    echo "$fqbn"
}

# Classify detected serial ports
S3_PORT=""
B_PORT=""
B_FQBN=""
for p in "${PORTS[@]}"; do
    if is_s3_port "$p"; then
        S3_PORT="$p"
    else
        # First non-S3 serial wins as Node B candidate.
        if [ -z "$B_PORT" ]; then
            B_PORT="$p"
            B_FQBN=$(get_fqbn "$p")
        fi
    fi
done

if [ -z "$S3_PORT" ] && [ -z "$B_PORT" ]; then
    echo "Error: no serial boards detected at all."
    echo "Plug in the S3 dongle (Node A) and the A1S Audio Kit (Node B) and re-run."
    exit 1
fi

if [ -z "$S3_PORT" ]; then
    cat <<EOF
Warning: no S3 (ESP32-S3 USB-JTAG/serial, VID 0x303a:0x1001) serial port detected.
This is expected if Node A is already running USB-audio firmware (the audio
interface hides /dev/ttyACM0).

To flash Node A:
  1. Hold the BOOT button on the S3
  2. Tap RST
  3. Release BOOT
  4. Re-run this script (or run 'pio run -d $NODE_A_PROJECT -t upload')
EOF
    if [ -z "$B_PORT" ]; then
        exit 1
    fi
    echo "Continuing with Node B flash only..."
    SKIP_NODE_A=1
fi

echo "------------------------------------------------"
if [ -z "$SKIP_NODE_A" ]; then
    NAME_A=$(get_board_name "$S3_PORT")
    echo "Step 1: Flashing Node A (S3, ESP-IDF UAC mic bridge) to $S3_PORT [$NAME_A]..."
    echo "Project: $NODE_A_PROJECT"
    if pio run -d "$NODE_A_PROJECT" -t upload --upload-port "$S3_PORT"; then
        echo "Node A flashed."
    else
        echo "Error: PlatformIO upload of Node A failed."
        echo "Hint: if upload fails to enter download mode, hold BOOT, tap RST,"
        echo "      release BOOT, then re-run."
        exit 1
    fi
else
    echo "Step 1: SKIPPED (no S3 serial port visible)."
fi

echo "------------------------------------------------"
if [ -z "$B_PORT" ]; then
    echo "Error: no Node B serial port detected. Cannot flash Node B."
    exit 1
fi

NAME_B=$(get_board_name "$B_PORT")
echo "Step 2: Flashing Node B (A1S Mic Test) to $B_PORT [$NAME_B]..."
echo "Compiling Node_B_Mic_Test with FQBN: $B_FQBN"
BUILD_B="/tmp/arduino_build_mic_B"
mkdir -p "$BUILD_B"
if arduino-cli compile --clean --build-path "$BUILD_B" --fqbn "$B_FQBN" "$NODE_B_SKETCH"; then
    echo "Uploading Node B Mic Test..."
    arduino-cli upload -p "$B_PORT" --input-dir "$BUILD_B" --fqbn "$B_FQBN" "$NODE_B_SKETCH"
else
    echo "Error: Compilation of Node B Mic Test failed."
    exit 1
fi

echo "------------------------------------------------"
echo "Success! Microphone Test nodes flashed."
echo
echo "Verify on Linux:"
echo "  arecord -l                                   # Note the card name for 'ESP UAC Device'"
echo "  arecord -D plughw:CARD=Device,DEV=0 \\"
echo "          -f S16_LE -r 16000 -c 1 -d 5 /tmp/mic.wav"
echo "  aplay /tmp/mic.wav"
echo
echo "LED on Node A: white boot blinks -> blue pulse (no packets) -> green (packets flowing)"
