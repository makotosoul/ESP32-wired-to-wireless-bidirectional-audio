...
#define CFG_TUD_AUDIO_FUNC_1_EP_IN_SW_BUF_SZ      CFG_TUD_AUDIO_FUNC_1_FORMAT_1_EP_SZ_IN * (MIC_INTERVAL_MS * 3)
...
// Number of Standard AS Interface Descriptors. Must equal the number of AS
// interfaces in the descriptor: 1 when only mic OR only speaker is enabled,
// 2 when BOTH are enabled (headset). Hardcoding to 1 here caused TinyUSB to
// silently ignore the speaker AS interface in dual-direction builds —
// uac_output_cb would never fire (LED stuck green / no spk_uac bytes).
